--replaces function in 041 to use the new mkr.balances_v2 table
...
begin
  select amount into wallet_amount from mkr.balances_v2 ba
  ...
  if proxy is not null then
    select amount into hot_wallet_amount from mkr.balances_v2 ba
    ...
    if proxy.hot != proxy.cold then
      select amount into cold_wallet_amount from mkr.balances_v2 ba
